...
'''
Overall training score: 0.02320348657667637
Accuracy on train set: 0.9920499920845032
Overall test score: 0.8021031022071838
Accuracy on test set: 0.8916000127792358
'''
import os

os.environ['KMP_DUPLICATE_LIB_OK']='True'
class FASHIONMNIST_DEEPCHECK_VERYDEEP(fashionmnist_dataset):

    def __init__(self):
        super(FASHIONMNIST_DEEPCHECK_VERYDEEP, self).__init__()
    ...
    def create_model(self, input_shape):
        ...
        model.add(Dense(128, name='dense_2', input_dim=input_shape))
        model.add(Activation('relu', name='relu_2'))

        model.add(Dense(64, name='dense_3'))
        model.add(Activation('relu', name='relu_3'))

        model.add(Dense(64, name='dense_4'))
        model.add(Activation('relu', name='relu_4'))

        model.add(Dense(32, name='dense_5'))
        model.add(Activation('relu', name='relu_5'))

        model.add(Dense(32, name='dense_6'))
        model.add(Activation('relu', name='relu_6'))

        model.add(Dense(16, name='dense_7'))
        model.add(Activation('relu', name='relu_7'))

        model.add(Dense(16, name='dense_8'))
...
if __name__ == '__main__':
    ...
    # train model
    if model == 'TRAIN':
        mnist = FASHIONMNIST_DEEPCHECK_VERYDEEP()
        mnist.set_num_classes(10)

        mnist.train_model(train=True,

                          kernel_path=f'{SRC_ROOT}/saved_models/fashionmnist/fashionmnist_deepcheck_verydeep/fashionmnist_deepcheck_verydeep.h5',
                          model_path=f'{SRC_ROOT}/saved_models/fashionmnist/fashionmnist_deepcheck_verydeep/fashionmnist_deepcheck_verydeep.json',
    ...
    if model == 'LOAD':
        # load model
        name_model = "fashionmnist_deepcheck_verydeep"
