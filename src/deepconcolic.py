...
def get_position(suspicious):  # suspicious: [l{layer index}_{neuron index}]
    layer_idx = []
    neuron_idx = []
    for sus in suspicious:
        neuron_idx.append(int(sus.split("_")[1]))
        layer_idx.append(int(sus.split("_")[0][1:]))  # ignore 'l'
    return np.asarray(layer_idx), np.asarray(neuron_idx)


def isValidNeuron(current_layer_idx, current_pos, suspicious):
    if suspicious is None:
        return True

    sus_layer_idx, sus_neuron_idx, max_suspicious_layer = suspicious

    if current_layer_idx < max_suspicious_layer:
        return True

    elif current_layer_idx == max_suspicious_layer:
        for index, kdx in enumerate(sus_layer_idx):
            if kdx == max_suspicious_layer:
                if sus_neuron_idx[index] == current_pos:
                    return True
    else:
        return False


def create_constraint_between_layers(model_object, modified_features, x, suspicious):
    ...
    for current_layer_idx, current_layer in enumerate(model.layers):
        if suspicious is not None and current_layer_idx > max_suspicious_layer:
            print(f"Ignore layer index {current_layer_idx} (using fault localization)")
            break
        ...
        if keras_layer.is_dense(current_layer):
            ...
            for current_pos in range(hidden_units_curr_layer):
                ...
                isOk = isValidNeuron(current_layer_idx, current_pos, suspicious)
                if not isOk:
                    continue
            ...
            for unit_idx in range(units):
                ...
                isOk = isValidNeuron(current_layer_idx, unit_idx, suspicious)
                if not isOk:
                    smt_constraints.append(f'\n; invalid')
                    continue
...
def create_variable_declarations(model_object, modified_features,
                                 suspicious,
                                 type_feature=get_config(["constraint_config", "feature_input_type"])):
    assert (isinstance(model_object, abstract_dataset))
    constraints = []
    model = model_object.get_model()
    ...
    constraint = ''
    for feature_idx in modified_features:
        if type_feature == 'float':
            ...
            print(f'Does not support the type {type_feature}')
    ...
    for layer_idx, layer in enumerate(model.layers):
        ...
        if units is not None:
            for unit_idx in range(units):
                # before_activation = f'(declare-fun l_{layer_idx}_{unit_idx} () Real)'
                # after_activation = f'(declare-fun v_{layer_idx}_{unit_idx} () Real)'
                # constraints.append(before_activation + '\t\t\t' + after_activation)
                if isValidNeuron(layer_idx, unit_idx, suspicious):
                    constraints.append(f'(declare-fun l{layer_idx}_{unit_idx} () Real)')
...
def create_feature_constraints_from_an_observation(model_object,
    ...
    if strategy == 'change_nonzero_pixels':
        for feature_idx in range(n_features):
            if x_train_shape784_bound255[feature_idx] == 0:  # fix value
                ...
                smt_constraints.append(smt_constraint)
                modified_features.append(feature_idx)
    ...
    return smt_constraints, modified_features


def create_suspicious_constraints_from_an_observation(model_object, x_train, y_train, thread_config):
    suspicious_neurons_Tarantula = ["l4_7", "l0_6"] # XXXX
    suspicious_neurons = suspicious_neurons_Tarantula

    # compute value of neurons
    presoftmax = -2
    neurons_dict = {}
    idx = -1
    for layer in model_object.get_model().layers[:presoftmax]:
        ...
        if keras_layer.is_dense(layer):
            ...
            neurons = intermediate_layer_model.predict(x_train.reshape(-1, 784))
            jdx = -1
            for neuron in neurons[0]:
                jdx += 1
                key = f"l{idx}_{jdx}"
                neurons_dict[key] = neuron

    # add constraint
    constraints = None
    smt_constraints = []
    for suspicious in suspicious_neurons:
        if suspicious not in neurons_dict:
            smt_constraints.append(f"; not found {suspicious}\n")
            continue

        if neurons_dict[suspicious] > 0:
            smt_constraints.append(f"(assert (< {suspicious} 0))")  # try to negate
        else:
            smt_constraints.append(f"(assert (> {suspicious} 0))")

    return constraints, smt_constraints, suspicious_neurons
...
def create_output_constraints_from_an_observation(model_object, x_train, y_train, thread_config):
    ...
    last_layer_idx = len(model.layers) - 2  # len - 1: presoftmax layer
    ...
    left = f'l{last_layer_idx}_{largest_idx}'
        ...
        if smt_type_constraint == 'or':
            ...
            for class_idx in range(n_classes):
                if class_idx != largest_idx:
                    if class_idx == 0:
                        smt_constraint = f'(< {left} l{last_layer_idx}_{class_idx}) '
                    else:
                        smt_constraint = f'(or {smt_constraint} (< {left} l{last_layer_idx}_{class_idx})) '
            smt_constraint = f'(assert {smt_constraint})'
            print(f'Output constraint = {smt_constraint}')
            smt_constraints.append(smt_constraint)
            ...
            smt_constraint = f'(assert (< {left} l{last_layer_idx}_{second_idx}) )'
            print(f'Output constraint = {smt_constraint}')
            smt_constraints.append(smt_constraint)
...
def create_constraints_file(model_object, seed_index, thread_config):
    ...
    smt_input_constraints_modification, modified_features = create_feature_constraints_from_an_observation(model_object,
                                                                                                           x_train,
                                                                                                           y_train,
                                                                                                           delta_lower_bound,
                                                                                                           delta_upper_bound,
    ...
    output_constraints, smt_output_constraints, suspicious = create_suspicious_constraints_from_an_observation(
        model_object,
        x_train,
        y_train,
        thread_config)

    # for idx in range(len(smt_output_constraints)):
    #     sus = suspicious[idx]
    #     sus_layer_idx, sus_neuron_idx = get_position([sus])


    sus_layer_idx, sus_neuron_idx = get_position(suspicious)
    max_suspicious_layer = np.max(sus_layer_idx)
    print(f"suspicious layer idx: {sus_layer_idx}")
    print(f"suspicious neuron idx: {sus_neuron_idx}")


    variable_types = create_variable_declarations(model_object, modified_features,
                                                  (sus_layer_idx, sus_neuron_idx, max_suspicious_layer))

    smt_layers_constraints = create_constraint_between_layers(model_object, modified_features,
                                                              x_train.reshape(-1),
                                                              (sus_layer_idx, sus_neuron_idx, max_suspicious_layer))
    ...
    with open(thread_config.constraints_file, 'w') as f:
        ...
        f.write('\n')
        # f.write(smt_output_constraints[idx])
    ...
    print(f'{thread_config.thread_name}: call SMT-Solver to solve the constraints')
    command = f"{thread_config.z3_path} -smt2 {thread_config.constraints_file} > {thread_config.z3_solution_file}"
    print(f'\t{thread_config.thread_name}: command = {command}')
    os.system(command)
    smttime.append(time.time() - start_smt_time)

    # parse solver solution
    print(f'{thread_config.thread_name}: parse solver solution')
    ...
    # print(f'{thread_config.thread_name}: \t{command}')
    ...
    # comparison
    try:
        x_train, _ = model_object.get_an_observation(seed_index)
        candidate_adv = analyze_smt_output(solution_path=thread_config.z3_normalized_output_file,
                                           img=np.round(x_train.reshape(-1) * 255))
    except:
        candidate_adv = None
        print("BUG IN ANALYZING SMT OUTPUT")
    ...
    if candidate_adv is not None:
        ...
        if is_valid:
            print(f'The constraints have VALID solution')
            with open(thread_config.selected_seed_index_file_path, mode='a') as f:
                f.write(str(seed_index) + ',')
            return
        else:
            print(f'The constraints have INVALID solution')
            os.remove(candidate_adv_csv_path)
    else:
        print(f'The constraints have no solution')
    print('--------------------------------------------------')
...
def image_generation(seeds, thread_config, model_object):
    ...
    for seed_index in seeds:
        if seed_index in analyzed_seed_indexes:
            print(f'{thread_config.thread_name}: Visited seed {seed_index}. Ignore!')
            continue
        else:
            '''
            ...
            print(f'{thread_config.thread_name}: seed index = {seed_index}')
            with open(thread_config.seed_index_file, mode='w') as f:
                f.write(str(seed_index))

            # generate constraints
            print(f'{thread_config.thread_name}: generate constraints')
...
def generate_samples(model_object):
    ...
    print("Before prioritization: len seeds = " + str(len(seeds)))
    seeds = priority_seeds(seeds, model_object)
    print("After prioritization: len seeds = " + str(len(seeds)))
    ...
    if n_threads >= 2:
        ...
        print(f'n_single_thread_seeds = {n_single_thread_seeds}')
        threads = []
    ...
    for seed_index in all_seeds:
        ...
        if seed_index not in selected_seed_index_arr:
            ...
            if pred_label == adv_label or pred_label != y_true[seed_index]:  # just confirm
                print(f"PROBLEM with seed {seed_index}! Its label {adv_label}")
