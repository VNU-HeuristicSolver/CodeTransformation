'''
Get all .csv file in a folder.
Construct an image for each csv file if its label is different from a specific label.
'''
import csv
import numpy as np
import os
...
if __name__ == '__main__':
    BASEPATH = f"/Users/ducanhnguyen/Documents/NpixelAttackDeepFault/datatest/proposal/mnist_ann_keras/out(K=-50.0,target-1)"
    NUM_ADV = 499
    NAME_MODEL = "mnist_ann_keras"
    ...
    for index in range(0, 2495):
        ...
        if os.path.exists(adv_path):
            ...
            if pred_ori_label == true_label:
                ...
                print(
                    f"Ignore: trueLabel = {true_label} ({pred_ori[0][true_label]}), pred_label = {pred_ori_label} ({pred_ori[0][pred_ori_label]})")

    print(f"success rate = {len(l0s) / NUM_ADV}")
